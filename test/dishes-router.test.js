...
describe("dishes router", () => {
  ...
  describe("create method", () => {
    test("creates a new dish and assigns id", async () => {
      ...
      const dishes = await request(app)
        .get(ATTACHED_PATH)
        .set("Accept", "application/json");
      expect(dishes.body.error).toBeUndefined();
      expect(dishes.body.data).not.toBeUndefined();
      expect(dishes.body.data.find(dish => dish.name === expectedName)).not.toBeUndefined();
    });
